...
Ydrop 是一个 **跨端**（Android + Web）个人快速采集 inbox，集 **随手记 · AI 整理 · 待办提取 · 语音转写 · 提醒日历 · WebDAV 同步** 于一体。文字、语音、悬浮窗速记、桌面快捷录音、桌面浏览器——所有入口汇进同一个收件箱，再由 AI 把碎片变成可执行内容。

**关键词**：`AI 整理` · `待办提取` · `随手记` · `闪念胶囊` · `语音转写` · `悬浮窗速记` · `提醒日历` · `WebDAV 同步` · `多端协同`
...
┌───────────────────┐      ┌──────────────────────┐
│  Android App      │      │  Web 浏览器          │
│  Kotlin / Compose │      │  Next.js / Tailwind  │
└────────┬──────────┘      └──────────┬───────────┘
         │                            │
         │ WebDAV                     │ HTTP API
         ▼                            ▼
┌───────────────────┐      ┌──────────────────────┐
│   NAS / 网盘      │◄────►│  relay_service        │
│   Markdown 文件   │ 双向 │  FastAPI + SQLite    │
└───────────────────┘ 同步 │  + AI 分析 + 转写     │
                           └──────────────────────┘
```

- **Android 端**：完整的移动体验（录音、悬浮窗、AI 整理、提醒、直连 WebDAV）
- **Web 端**：桌面浏览器管理所有笔记，通过 `relay_service` 后端 CRUD
- **WebDAV**：多端数据源（Markdown + frontmatter），Android 和后端各自双向同步
- **relay_service**：AI 分析网关 + 转写中转 + Web 端 CRUD API + 静态页面服务
...
| 层 | 技术 |
|------|------|
| Android | Kotlin · Jetpack Compose + Material3 · Room · DataStore · Coroutines · WorkManager · OkHttp |
| Web 前端 | Next.js 14 (App Router, 静态导出) · React · Tailwind CSS · SWR · Zustand · lucide-react |
| 后端 | FastAPI · aiosqlite · httpx · pydantic · uvicorn |
| 同步 | WebDAV 双向同步（Android & 后端均实现）· Markdown + YAML frontmatter |
| AI | OpenAI / Anthropic 兼容网关 · Relay 中转 · 多模式探测 |
| 构建 | AGP 8.5.2 · Kotlin 2.0.21 · JVM 17 · Python 3.11 · Node.js 20+ |
...
### Web 端（relay_service + Next.js）

一键启动：
```bash
cd relay_service
cp .env.example .env      # 编辑填入 RELAY_TOKEN 和 WEBDAV_*
./run.sh
```

浏览器访问 `http://localhost:8787/` ，首次进入在设置页输入 `RELAY_TOKEN`。
...
# 2. 后端启动（需 Python 3.11+）
python3.11 -m venv .venv
.venv/bin/pip install -r requirements.txt
RELAY_TOKEN=your-token .venv/bin/uvicorn app.main:app --host 0.0.0.0 --port 8787
```
